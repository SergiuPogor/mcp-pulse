...
const CallsView = {
  render() {
    ...
    const filtered = state.calls.filter((c) => {
      ...
      if (state.filters.tool && !c.tool.toLowerCase().includes(state.filters.tool.toLowerCase()))
        return false
      return true
    })
...
const AlertsView = {
  ...
  renderConfigs() {
    ...
    if (!el) return
    el.innerHTML =
      '<p style="color:#4a5068;font-size:.82rem;margin-bottom:12px">Config editor not yet implemented in this preview.</p>'
  },
  addConfig() {},
  saveConfig() {
    this.closeConfig()
...
function updateServersBadge() {
  ...
  if (badge)
    badge.textContent = state.servers.filter((s) => s.status === 'connected').length.toString()
}
