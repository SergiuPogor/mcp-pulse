...
export class PulseStore {
    ...
    const sanitized: MCPCall = {
      ...
      request: {
        ...
        params: call.request.params
          ? (redact(call.request.params) as Record<string, unknown>)
          : undefined,
      },
  ...
  getCallsCount(
    opts: Partial<{ serverId: string; tool: string; status: CallStatus }> = {},
  ): number {
  ...
  getMetrics(since?: string): MetricsSummary {
    ...
    for (const call of calls) {
      ...
      existing.avgLatency =
        (existing.avgLatency * (existing.calls - 1) + call.duration) / existing.calls
  ...
  private updateServerOnCall(call: MCPCall): void {
    ...
    server.avgLatency =
      prevCalls > 0 ? (prevAvg * prevCalls + call.duration) / server.calls : call.duration
  ...
  private trim(): void {
    ...
    const entries = Array.from(this.calls.entries()).sort((a, b) =>
