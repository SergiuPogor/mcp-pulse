...
function printBanner() {
  ...
  console.log(chalk.blue('  │') + chalk.bold.white('  🔴 mcp-pulse') + chalk.dim(' — MCP Observability    ') + chalk.blue('│'));
  console.log(chalk.blue('  │') + chalk.dim(`  v${pkg.version}  ·  MIT License               `) + chalk.blue('│'));
  console.log(chalk.blue('  └─────────────────────────────────────────┘'));
  console.log('');
}
...
program
  ...
  .action(async (opts) => {
    ...
    console.log(chalk.dim(`  → Web UI:         `) + chalk.white.underline(`http://localhost:${port}`));
...
program
  ...
  .action(async (opts) => {
    ...
    console.log('');
    console.log(chalk.dim('  Point your MCP client at: ') + chalk.cyan(`http://localhost:${opts.port}/sse`));
    console.log('');
  ...
  .action(async (opts) => {
    ...
    const checkAlerts = async () => {
      try {
        ...
        console.log(chalk.yellow(`[mcp-pulse alert] Cannot reach dashboard: ${(err as Error).message}`));
      }
    };
